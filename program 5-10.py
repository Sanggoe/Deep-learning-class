# -*- coding: utf-8 -*-
"""
Created on Tue May 24 09:12:41 2022
...
dmlp_mse.add(Dense(units=n_hidden1, activation='tanh'))
...
hist_mse=dmlp_mse.fit(x_train, y_train, batch_size=128, epochs=30,
             validation_data=(x_test, y_test), verbose=2)

# 교차 엔트로피를 사용한 모델
dmlp_ce=Sequential()
dmlp_ce.add(Dense(units=n_hidden1, activation='tanh', input_shape=(n_input,)))
dmlp_ce.add(Dense(units=n_hidden1, activation='tanh'))
...
hist_ce=dmlp_ce.fit(x_train, y_train, batch_size=128, epochs=30,
             validation_data=(x_test, y_test), verbose=2)
...
# 정확률 곡선
plt.plot(hist_mse.history['accuracy'])     # train set에 대한 정확성 그래프
plt.plot(hist_mse.history['val_accuracy']) # test set에 대한 정확성 그래프
plt.plot(hist_ce.history['accuracy'])     # train set에 대한 정확성 그래프
plt.plot(hist_ce.history['val_accuracy']) # test set에 대한 정확성 그래프
plt.title('Model accuracy comparison between MSE and cross entropy')
plt.ylabel('Accuracy')
plt.xlabel('Epoch')
plt.legend(['Train_mse', 'Validation_mse', 'Train_ce', 'Validation_ce'], loc='best') # 범례, loc(위치): 가장 좋은 위치에 
plt.grid()                                            # grid 그리기
plt.show()                                            # 그래프 그리기