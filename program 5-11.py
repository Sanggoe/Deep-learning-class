# -*- coding: utf-8 -*-
"""
Created on Tue May 24 09:45:51 2022
...
# 하이퍼 매개변수 설정
batch_size = 256 # 한 번에 처리하는 샘플의 개수
n_epoch = 50    # 반복 횟수
...
hist_sgd=dmlp_sgd.fit(x_train, y_train, batch_size=128, epochs=30,
             validation_data=(x_test, y_test), verbose=2)

# Adam 옵티마이저를 사용하는 모델
dmlp_adam = build_model()
dmlp_adam.compile(loss="categorical_crossentropy", optimizer=Adam(), metrics=['accuracy'])
hist_adam=dmlp_sgd.fit(x_train, y_train, batch_size=128, epochs=30,
             validation_data=(x_test, y_test), verbose=2)
...
hist_adagrad=dmlp_sgd.fit(x_train, y_train, batch_size=128, epochs=30,
             validation_data=(x_test, y_test), verbose=2)
...
hist_rmsprop=dmlp_sgd.fit(x_train, y_train, batch_size=128, epochs=30,
             validation_data=(x_test, y_test), verbose=2)
...
plt.legend(['Train_sgd', 'Val_sgd', 'Train_adam', 'Val_adam',
            'Train_adagrad', 'Val_adagrad', 'Train_rmsprop', 'Val_adam'], loc='best') # 범례, loc(위치): 가장 좋은 위치에 
plt.grid()
plt.show()